...
from .models import Event, StudentEvent, WroteTime, CustomUser, Group, EventGroup, TeacherSubjects, TeacherSubjectsEvent, UserGroup
from django.db.models import Q

class EventService:
    """Сервис для управления мероприятиями, включая проверку пересечений и создание/обновление мероприятий"""
    @staticmethod
    ...
    def update_event(event_id, data, teacher, groups=None):
        """
        Обновление мероприятия с проверкой пересечений (только предупреждение)
        """
        event = Event.objects.get(id=event_id)
        start_time = data.get('start_time', event.start_time)
        end_time = data.get('end_time', event.end_time)
        
        event_teacher = TeacherSubjectsEvent.objects.filter(
            event=event
        ).first()  
        
        if event_teacher and event_teacher.teacher_subject.teacher != teacher and teacher.role != 'admin':
            raise ValidationError(
                "Вы не можете редактировать это мероприятие"
            )
