...
class CustomUser(AbstractUser):
    ...
    role = models.CharField(max_length=15, choices=ROLE_CHOICES, default='student', db_index=True)
...
class TeacherSubjectsEvent(models.Model):
    teacher_subject = models.ForeignKey(TeacherSubjects, on_delete=models.CASCADE, related_name="events")
    event = models.ForeignKey('Event', on_delete=models.CASCADE, related_name="teacher_assignments")
    created_at = models.DateTimeField(auto_now_add=True)
...
class StudentFlow(models.Model):
    student = models.ForeignKey(CustomUser, on_delete=models.CASCADE, related_name="student_flows")
    flow = models.ForeignKey(Flow, on_delete=models.CASCADE, related_name="students_in_flow")
    created_at = models.DateTimeField(auto_now_add=True)
...
class WroteTime(models.Model):
    ...
    event = models.ForeignKey(Event, on_delete=models.CASCADE, related_name="written_times")
...
class StudentEvent(models.Model):
    event = models.ForeignKey(Event, on_delete=models.SET_NULL, null=True, blank=True, related_name="student_events")
    time = models.OneToOneField(WroteTime, on_delete=models.CASCADE, related_name="student_event")
    ...
    user = models.ForeignKey(CustomUser, on_delete=models.CASCADE, related_name="student_events")
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class UserGroup(models.Model):
    user = models.ForeignKey(CustomUser, on_delete=models.CASCADE, related_name="user_groups")
...
class ProgramSubjects(models.Model):
    ...
    subject = models.ForeignKey(Subject, on_delete=models.PROTECT, related_name="subject_programs")
    created_at = models.DateTimeField(auto_now_add=True)
